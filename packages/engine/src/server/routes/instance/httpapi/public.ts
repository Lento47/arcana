...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for arcana serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(ArcanaHttpApi)
...
export const PublicApi = ArcanaHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "arcana",
    version: "1.0.0",
    description: "arcana api",
