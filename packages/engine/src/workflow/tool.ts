...
const GEN_PROMPT = [
  ...
  "  \"sources\"?: string[] (for merge), \"condition\"?: string, \"ifTrue\"?: string[], \"ifFalse\"?: string[] }.",
  ...
  "Conditions are a restricted expression language, NOT JavaScript: prior step ids as variables, quoted strings, numbers,",
  "comparisons (== != < > <= >=), logic (! && ||), arithmetic (+ - * / %), and only these members: includes(x),",
  "startsWith(x), endsWith(x), .length. No function calls, imports, or other property access — anything else fails the condition.",
