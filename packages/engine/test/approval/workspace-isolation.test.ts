...
describe("runtime API workspace isolation", () => {
  it.instance(
    "operator A cannot list workspace B sessions",
    () =>
      Effect.gen(function* () {
    ...
    // Server-boot tests are slow under full-suite load; 5s default times out.
    30_000,
  )

  it.instance(
    "operator A cannot decide workspace B approvals, even with a directory query",
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const wsA = yield* seedWorkspace(test.directory)
  ...
  it.instance(
    "operator A cannot retrieve workspace B proofs",
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const wsA = yield* seedWorkspace(test.directory)
  ...
  it.instance(
    "query parameters cannot broaden authenticated scope for approval reads",
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const wsA = yield* seedWorkspace(test.directory)
  ...
  it.instance(
    "within one workspace, session A still cannot decide session B's approval",
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
