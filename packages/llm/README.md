...
Schema-first LLM core for Arcana. One typed request, response, event, and tool language; provider quirks live in adapters, not in calling code.

```ts
import { Effect } from "effect"
import { LLM, LLMClient } from "@arcana/llm"
...
```ts
import { OpenAI, CloudflareAIGateway } from "@arcana/llm/providers"
