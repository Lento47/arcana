...
describe("resource", () => {
  ...
  test("drops OTEL resource attributes when any entry is invalid", () => {
    ...
    expect(resource().attributes["arcana.client"]).toBeDefined()
  })

  test("keeps built-in attributes when env values conflict", () => {
    process.env.ARCANA_CLIENT = "cli"
    process.env.OTEL_RESOURCE_ATTRIBUTES =
      "arcana.client=web,service.instance.id=override,service.namespace=anomalyco"
    ...
    expect(resource().attributes["arcana.run"]).toMatch(/^[0-9a-f]{8}$/)
  })
})
