...
describe("ReadTool", () => {
  ...
  it.effect("registers, authorizes, and reads through the location filesystem", () =>
    Effect.gen(function* () {
      ...
      expect(readCalls).toEqual([{ input: AbsolutePath.make(path.resolve("README.md")), page: {} }])
    }),
  )
  ...
  it.effect("returns a small PNG as native media instead of durable base64 text", () =>
    Effect.gen(function* () {
      ...
      expect(readCalls).toEqual([{ input: AbsolutePath.make(path.resolve("pixel.png")), page: {} }])
  ...
  it.effect("preserves unexpected filesystem defects", () =>
    Effect.gen(function* () {
      ...
      expect(readCalls).toEqual([
        { input: AbsolutePath.make(path.resolve("archive.dat")), page: { offset: 2, limit: 1 } },
      ])
    }),
  )
  ...
  it.effect("forwards pagination and returns bounded text pages with continuation", () =>
    Effect.gen(function* () {
      ...
      expect(readCalls).toEqual([
        { input: AbsolutePath.make(path.resolve("large.txt")), page: { offset: 2, limit: 1 } },
      ])
    }),
  )
