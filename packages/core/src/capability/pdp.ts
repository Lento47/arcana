...
/** Informational notices that never change the decision kind. */
export type NoticeReasonCode = "MISSING_TOOL_REASON"
...
export function evaluate(
  ...
  // Operator-facing reason: requested by every model-facing tool schema and
  // shown in the approval gate when present. Its absence must NOT hard-block a
  // call — the operator gate (remote content, risk, provenance) still decides,
  // and a dead-end deny leaves the model no path forward.
  if (intentRisk !== "LOW" && request.provenance.includes("MODEL_OUTPUT") && !request.reason?.trim()) {
    reasons.push({
      code: "MISSING_TOOL_REASON",
      message: "Tool call did not supply a reason; the approval gate will show (no reason provided)",
      severity: "info",
    })
  }
