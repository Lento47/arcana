...
describe("LRU write lives in a createEffect keyed on the memo", () => {
  test("effect reads the memo result", () => {
    expect(projectionSrc).toContain("const result = entries()")
  })

  test("effect persists the cache write", () => {
    expect(projectionSrc).toContain("state.cache = result.cache")
    expect(projectionSrc).toContain("state.previousEntries = result.entries")
  })
