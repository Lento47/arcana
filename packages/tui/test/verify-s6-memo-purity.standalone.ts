 ...
 *
 * The `entries` createMemo (originally in command-spine-shell.tsx, moved to
 * use-spine-projection.ts by the PR5 split) called
...
const memoMatch = projectionSrc.match(/const entries = createMemo\(\(\) => \{([\s\S]*?)\n  \}\)/)
...
check(projectionSrc.includes("const result = entries()"), "effect reads the memo result")
check(projectionSrc.includes("state.cache = result.cache"), "effect persists the cache write")
check(projectionSrc.includes("state.previousEntries = result.entries"), "effect persists the previousEntries write")
check(
  projectionSrc.indexOf("createEffect(() => {\n    const result = entries()") >= 0,
  "effect is keyed on the memo result (tracking read at effect top)",
)
check(projectionSrc.includes("entries().entries"), "visibleEntries derives from the result's entries array")
