...
- Collapsed subagent rows are a compact strip, not a box: a state glyph (`●` running / `✓` done — the header chip already names the actor, so the strip carries progress, not `delegated`/`returned`) · step count · the newest step label as the live activity · sibling position (`2/5` when the wave has siblings) · a right-aligned `↵ open` badge behind a dim `· · ·` lead-in that is itself the click target (it navigates to the child; the header above does too). Live relay lines and the returned one-line preview render under the strip; **no placeholder body** — a quiet running card is the strip alone. Five framed one-liners were five empty boxes.
