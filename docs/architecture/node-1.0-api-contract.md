...
| D-5 | Emergency push (SSE/WS) beyond pull-based polling | **DONE 2026-08-02** — `GET /api/sync/revocations/stream` SSE pushes published revocation statements to per-directory subscribers with per-connection sequence; publish and emergency-deny both broadcast; subscribers must still verify the signed statements |
| D-4 | DELTA bundles + compatibility negotiation | **DELTA transport DONE 2026-08-02** — control plane serves POLICY_DELTA (one-step gap with matching base digest) and REVOCATION_DELTA (statements after accepted sequence, bounded at 32); core verification fails closed to FULL_SNAPSHOT_REQUIRED; **node-side sync client validates deltas before acceptance** (base/sequence/result/target consistency, contiguous revocation statements) and **`arcana node sync` persists accepted policy/revocation state durably** (`sync-state.ts`; resumed from persisted state). **Compatibility negotiation DONE 2026-08-02** — served policy responses carry `compatibleFrom`/`compatibleTo`; the node client rejects a bundle whose range excludes its supported version, and fails closed when the range is missing. |
