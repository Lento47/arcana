...
| BLK-F-AUTH-01 | P0 authenticated administrative identity binding | Enterprise administrative mutations must derive actor and tenant identity from authenticated server context. Client-supplied `actorUserId`, `approvedBy`, `tenantId`, or equivalent body fields must not establish authority or audit attribution. Current state: enterprise mutation payloads accept client-supplied actor fields; RBAC decision core PASS, authenticated HTTP boundary NOT implemented. 2026-08-03: the runtime approval surface (approve/deny/revoke) closed the same pattern: operator identity is derived from the authenticated server context and body fields are rejected/ignored; the enterprise admin surface remains open. **IMPLEMENTED 2026-08-03 (closure evidence):** the authorization middleware now attaches the authenticated admin principal (Basic auth username when `ARCANA_SERVER_PASSWORD` is set, else trusted `local-operator` context) as a request-scoped service; all 36 enterprise mutation endpoints require a server-side tenant binding (403 otherwise, fail closed); authority-critical mutations (assignRole, decide/revoke/bulk-deny approvals, policy promotion, revocation campaigns) additionally require the derived principal's RBAC permission; client-supplied `actorUserId`/`approvedBy`/`requestedBy`/`who`/`actor` body fields were removed from the payload schemas (excess fields ignored) and cannot establish authority or audit attribution; `createOrganization` rejects claims on existing tenant ids and seeds the authenticated creator as OWNER. Adversarial fixtures fail closed: cross-tenant impersonation 403, tenant-id squatting 403, forged approver/requester ignored, audit/custody records use the authenticated principal (local context and Basic-auth `alice`). Evidence: identity unit suite 9/9, enterprise HTTP auth-boundary suite 3/3, enterprise HTTP suite 23/23, core enterprise suite 80/80, engine typecheck green | Authenticated principal → tenant → role → permission; body actor attribution rejected or ignored; cross-tenant impersonation fixtures fail closed; forged approver fixtures fail closed; audit records use the authenticated principal |
