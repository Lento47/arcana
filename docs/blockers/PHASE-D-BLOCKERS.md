...
| BLK-D-04 | D-8B remote proof registration | **Control-plane registration IMPLEMENTED 2026-08-02**: `proof-registration.ts` (fixed validation order: schema → batch root → payload → enrollment → trust domain → signature → duplicate → chain continuity), durable `SqliteProofBatchLedger`, HTTP `POST /api/proof/batches` + `GET /api/proof/nodes/:nodeId/reconcile` (RECONCILED/GAPS_DETECTED/MISMATCH), 16 core tests + 2 HTTP integration tests. Node registry is env-backed pending D-1 enrollment (BLK-D-05); node-side uploader/outbox and hostile-node duplicate-delivery matrix remain | Node/server hash reconciliation exercised over HTTP; node-side uploader integration; hostile-node matrix |
