...
| BLK-F-11 | F11 enterprise API and automation | **Admin HTTP surface MOUNTED 2026-08-02** (`/api/enterprise/*`: orgs, roles, fleet, approvals, audit; 1 HTTP integration test) + canonical admin events (2 tests). Remaining: Terraform provider, SIEM export, policy/federation endpoints | Admin API + webhooks + automation tested (core HTTP surface DONE; Terraform/SIEM pending) |
