...
| E3 TypeScript/JavaScript SDK 1.0 | 10% | PARTIAL | **Governance surface IMPLEMENTED 2026-08-02** (`@arcana/sdk/v2/governance`: canonical `buildAuthorizationRequest` with exact request hashing, `toAuthorizationRequest` framework-adapter hook, `verifySignedEnvelope` strict parse + Ed25519 verify; 3 tests, SDK suite 10/10; SDK now depends on `@arcana/core`; plugin/sdk tsconfigs aligned to bundler). Remaining: proof-verifier client for RunProof exports, stable error model + semver/compat policy, conformance pass | BLK-E-03 |
