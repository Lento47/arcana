...
| D5 Remote revocation | 15% | PARTIAL | envelopes + durable state + sync; transport now authenticated (D-6B-T); convergence measurement pending | BLK-D-01 |
| D6 Distributed replay resistance | 10% | PARTIAL | **Execution ledger IMPLEMENTED 2026-08-02** (`execution-ledger.ts` + SQLite + HTTP `/api/executions/claim|complete|unknown`): executionId+node+session+requestHash+grant+nonce binding, exactly-once claim, cross-node duplicate matrix, CONFLICT on identity change, UNKNOWN_AFTER_CRASH/NETWORK with REPLAY_FORBIDDEN for irreversible effects, usage-limit interplay (retry cannot consume a second use); 7 core + 2 engine tests. Remaining: wire claim-before-effect into the distributed PEP execution path | BLK-D-01 |
...
| D8 Cross-node proof composition | 10% | PARTIAL | D-8A batching (Merkle root + gap detection); **D-8B remote proof registration IMPLEMENTED 2026-08-02** — control-plane ledger (`proof-registration.ts` + `proof-registration-sqlite.ts`), HTTP `/api/proof/batches` + `/api/proof/nodes/:nodeId/reconcile`, **node-side uploader + durable outbox** (`proof-uploader.ts`, `proof-outbox-sqlite.ts`, engine HTTP client `proof-upload-client.ts`; exponential backoff + jitter, receipt idempotency, poison states, restart recovery), 27 core tests + 6 engine tests; remaining: scheduler/CLI wiring + hostile-node duplicate-delivery matrix | BLK-D-04 |
...
| AUD-23 | Wire the execution ledger claim-before-effect into the distributed PEP (local nodes register execution IDs before protected effects) | BLK-D-01/D-6 | Engineering |
