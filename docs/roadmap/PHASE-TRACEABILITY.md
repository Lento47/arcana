...
| D4 Policy distribution | PARTIAL | signed bundle store (publish/staged/last-known-good/rollback, 11 tests) + HTTP policy endpoints + POLICY_SNAPSHOT/POLICY_DELTA/REVOCATION_DELTA via sync transport (4 core + 4 engine tests) + node-side delta validation (3 engine tests); node runtime persistence + compat negotiation pending | BLK-D-01 |
