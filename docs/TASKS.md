...
| AUD-61 | Restore the full TUI and focused engine/core suites to green; the 42 TUI failures reproduce under pinned Bun 1.3.14 and Bun 1.4.0, so resolve or classify them as worktree/test-isolation regressions before exact-commit verification — **RESOLVED**: at base 00ab9050 the 42 `sdk.event.on` failures no longer reproduce (SDK event mock already fixed at base); the single remaining failure was a stale lexicon assertion, fixed in bf90fe5b (PR #147); full TUI suite verified 1217/0 at branch tip | BLK-TUI-08 | Engineering |
