...
## Repo ops & debugging quick facts

- `docs/CURRENT-STATE.json` is GENERATED by `bun scripts/generate-current-state.ts` — never hand-edit; CI (`status-authority.yml`) pins `implementationCheckpoint == HEAD`. Rich operator notes merge from optional sidecar `docs/CURRENT-STATE.extras.json`.
- Durable diagnostic logs live in `%TEMP%` and survive terminal death: `arcana-daemon.log` (boot/exit), `arcana-permission-debug.log` (gate store trace), `arcana-message-debug.log` (submit→echo→SSE latency). Read these before asking users to copy terminal scrollback.
- Stale-daemon check: per-call errors on symbols that exist on disk mean the daemon predates the edit (Bun dev never reloads). Compare process CreationDate vs file mtimes before debugging code.
- Killing the dev daemon is safe — the TUI's `wrapDaemonFetch` auto-respawns it via `ARCANA_DAEMON_CMD` and retries the request once.
- The dev TUI prefers a built `packages/engine/src/cli/tui/worker.js` over source if that file exists (`target()` in cli/cmd/tui.ts) — its presence means restarts serve stale bundles.
- Profiling env flags: `ARCANA_PROFILE_SESSION_SWITCH=1` ([session-switch]/[session-load] timings), `ARCANA_DEBUG_SUBMIT_TIMING=1` (composer stages), plus the two debug-log envs above for stderr mirroring.
