...
|---|---|---|---|
| BLK-D-01 | D-6B-T production authenticated transport | **Message-layer transport IMPLEMENTED 2026-08-02**: `sync-transport.ts` (Ed25519-signed request/response envelopes, nonce + expiry freshness, audience binding), `sync-replay-store-sqlite.ts`, HTTP `POST /api/sync/policy|revocation`, node `sync-client.ts`. **Policy delivery now real**: `POST /api/policy/bundles` publishes signed bundles (strict schema, chaining, staged activation, last-known-good, audited rollback) and the sync endpoint serves POLICY_SNAPSHOT to behind nodes. Remaining: TLS/mTLS transport encryption + channel binding (BLK-D-07), DELTA bundles + compatibility negotiation (D-4) | MITM fixtures fail at message layer (DONE); TLS/mTLS handshake fixtures + channel binding at deployment |
