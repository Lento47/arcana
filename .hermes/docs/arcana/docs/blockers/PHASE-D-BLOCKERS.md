...
| BLK-D-09 | Node 1.0 freeze | **API contract draft published 2026-08-02** (`docs/architecture/node-1.0-api-contract.md`: frozen HTTP/CLI/core surface + §31 gate evidence + outstanding gates). **DELTA bundles DONE 2026-08-02** (POLICY_DELTA/REVOCATION_DELTA served by the sync control plane with verification). Release freeze NOT authorized: TLS/mTLS (BLK-D-07), live Linux validation (BLK-D-03), optional push channel (D-5), independent reproduction pending | §31 gates all zero + milestone tag after outstanding gates close |
