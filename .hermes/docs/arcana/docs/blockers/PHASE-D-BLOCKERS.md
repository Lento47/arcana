...
| BLK-D-05 | Node enrollment and key rotation | **Control-plane enrollment implemented 2026-08-02**: `node-enrollment.ts` (join-token ceremony, NodeIdentityCertificate issuance, TRUSTED/SUSPENDED/REVOKED lifecycle, epoch-advancing key rotation, superseded-key rejection, decommissioning blocks re-enrollment) + `node-enrollment-sqlite.ts` (durable registry) + 13 tests incl. restart persistence and trust-domain key snapshots for the D-8B registry. Remaining: enrollment HTTP endpoints + node-side ceremony client and key store | Unknown-node rejection, rotated-key rejection, duplicate-enrollment detection exercised over endpoints; node-side key store restart-safe |
