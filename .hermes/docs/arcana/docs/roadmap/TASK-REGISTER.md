...
| D4 Policy distribution/versioning | 10% | PARTIAL | digest chains in envelopes; transport now authenticated (D-6B-T); signed policy bundle store + rollback rules pending | BLK-D-01 |
...
| D6 Distributed replay resistance | 10% | PARTIAL | reducers/durable state + transport replay protection (D-6B-T); cross-node duplicate-execution matrix pending | BLK-D-01 |
