...
| D8 Proof composition | PARTIAL | D-8A Merkle batching + gap detection; **D-8B remote registration** (`proof-registration.ts`, `proof-registration-sqlite.ts`, HTTP `/api/proof/*`, 18 tests) | BLK-D-04 |
