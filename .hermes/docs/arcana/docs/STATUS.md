...
| Phase D — Distributed Authority | ACTIVE DEVELOPMENT, ~45–55% by playbook weighting; D-7 frozen (`arcana-phase-d7-local-distributed-authority`), D-8A proof batching implemented, **D-8B proof registration end-to-end + D-9 offline grant/lease policy + D-1 enrollment w/ HTTP + D-6B-T signed-envelope sync transport + D-4 signed policy bundle store + D-6 execution ledger + governed distributed PEP + D-5 revocation store/convergence + emergency deny-list + D-10 hostile-node matrix + Node CLI + Node 1.0 API contract draft (2026-08-02)**; Node 1.0 release freeze pending (TLS, live Linux, proof-store integration, L3) |
